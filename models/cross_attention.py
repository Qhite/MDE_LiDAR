...
class Cross_Attention_Block(nn.Module):
    def __init__(self, cfg=None, feature_dim=[]):
        ...
        self.Conv = nn.Sequential(                                                          
            nn.Conv2d(feature_dim[-3], feature_dim[-3], kernel_size=3, stride=2, padding=1, bias=False), # depth-wise
            nn.BatchNorm2d(feature_dim[-3]),
            nn.LeakyReLU(),
            nn.Conv2d(feature_dim[-3], 1, kernel_size=3, stride=1, padding=1, bias=False), # point-wise
            nn.BatchNorm2d(1),
            nn.LeakyReLU(),
        )
        ...
        # Q, K, V Linear layers
        self.W_q = nn.Linear(self.W//16, self.d_model, bias=False) # self.W//4
    ...
    def forward(self, data_dict):
        ...
        batch_size = x.size(0)
        position = Positional_encoding(batch_size, 15, self.d_model).to(x.device) #self.H//2
        ...
        output = F.avg_pool2d(self.W_o1(res.unsqueeze(1)), (15, 1)).view(batch_size, -1) # self.H//4
